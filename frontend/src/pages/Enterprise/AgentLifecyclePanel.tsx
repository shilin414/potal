...
export default function AgentLifecyclePanel() {
  ...
  const createVersion = async () => { try { const values = await versionForm.validateFields(); for (const key of ['model_config','tool_config','skill_config','knowledge_config','guardrail_config','workflow_config']) values[key] = asJson(values[key], key.endsWith('_config') && ['tool_config','skill_config','knowledge_config'].includes(key) ? [] : {}); await api.post(`/agents/${agentId}/versions/`, values); message.success('版本草稿已创建'); setVersionOpen(false); await loadDetails(); } catch (error: any) { if (!error?.errorFields) message.error(error.message || 'JSON 格式错误'); } };
